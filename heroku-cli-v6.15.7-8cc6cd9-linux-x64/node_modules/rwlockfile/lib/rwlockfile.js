...
const is_process_active_1 = require("is-process-active");
...
class RWLockfile {
    ...
    check(type) {
        return tslib_1.__awaiter(this, void 0, void 0, function* () {
            ...
            if (status.status === 'open')
                ...
                if (!(yield is_process_active_1.isActive(status.job.pid))) {
                ...
                const pids = yield Promise.all(status.jobs.map((j) => tslib_1.__awaiter(this, void 0, void 0, function* () {
                    if (!(yield is_process_active_1.isActive(j.pid)))
                        return j.pid;
                })));
    ...
    checkSync(type) {
        ...
        if (status.status === 'open')
            ...
            if (!is_process_active_1.isActiveSync(status.job.pid)) {
            ...
            const inactive = status.jobs.map(j => j.pid).filter(pid => !is_process_active_1.isActiveSync(pid));
            if (inactive.length) {
